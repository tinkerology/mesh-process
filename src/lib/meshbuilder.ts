...
export class MeshBuilder {
    static buildConvexFanSurfaceWithEdgeVertices(
    ...
    static buildCircle(radius: number, facets: number): MeshInterface {
        ...
        for (let i = 0; i < facets; i++) {
            const edgeV1: VertexInterface = new Vertex(
                Math.cos(i * angle) * radius,
                Math.sin(i * angle) * radius,
                0
            );
            const edgeV2: VertexInterface = new Vertex(
                Math.cos((i + 1) * angle) * radius,
                Math.sin((i + 1) * angle) * radius,
                0
            );
