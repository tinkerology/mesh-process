...
export class MeshOperations {
    ...
    static removeTriangles(
        mesh: MeshInterface,
        trianglesToRemove: TriangleInterface[]
    ): MeshInterface {
        const removedMesh = new Mesh();

        mesh.triangles.forEach(function (triangle: TriangleInterface) {
            let found = false;
            trianglesToRemove.forEach(function (
                triangleToRemove: TriangleInterface
            ) {
    ...
    static crop(mesh: MeshInterface, boundingBox: BoundingBox): Scene {
        ...
        mesh.triangles.forEach(function (triangle: TriangleInterface) {
            if (
                triangle.v1.isInside(boundingBox) &&
    ...
    static translateFiltered(
        mesh: MeshInterface,
        vertexFilter: VertexFilterInterface
    ): MeshInterface {
        const translatedMesh: MeshInterface = new Mesh();
        mesh.triangles.forEach((triangle: TriangleInterface) => {
            translatedMesh.addTriangle(
    ...
    static filterVertices(
        mesh: MeshInterface,
        vertexFilter: VertexFilterInterface
    ): MeshInterface {
        const filteredMesh: MeshInterface = new Mesh();
        mesh.triangles.forEach((triangle: TriangleInterface) => {
            filteredMesh.addTriangle(
                new Triangle(
                    vertexFilter.filter(triangle.v1),
                    vertexFilter.filter(triangle.v2),
                    vertexFilter.filter(triangle.v3)
    ...
    static scaleToSize(mesh: MeshInterface, axisName: string, size: number) {
        ...
        const scaledMesh: MeshInterface = MeshOperations.scale(
            mesh,
            new Vertex(xScale, yScale, zScale)
        );
        return scaledMesh;
    }
    ...
    static replicate(
        ...
        while (totalCount > 0) {
            const translatedMesh = MeshOperations.translate(
                mesh,
                new Vertex(xOffset, yOffset, 0)
            );
    ...
    static addBaseAndVerticalTriangles(
        mesh: MeshInterface,
        extraZHeight: number
    ) {
        const extents: MeshExtents = MeshInfo.getExtents(mesh);
        ...
        const singleEdges: Edge[] = MeshInfo.getSingleEdges(
            MeshInfo.getEdges(mesh)
        );
